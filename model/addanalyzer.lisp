...
(defun load-data (&key ts low high its/p/v for-indexing? &aux first-fno last-fno label temp constrain-by-label)
  ...
  (with-open-file 
   (logsum (print (format nil "addstats/~a-logsum.lisp" ts))
...
(defun summarize-logs (ts)
  ...
  (loop for file being the hash-keys of *file->log*
	...
	do 
	(with-open-file 
	 (o (print (format nil "addstats/~a-~a-logsummary.xls" ts (substitute #\_ #\space (pathname-name file))))
	    :direction :output :if-exists :supersede) 
	 ;; output headers
...
(defun summarize-final-strategy-prefs (ts)
  ...
  (with-open-file 
   (o (print (format nil "addstats/~a-finalstrategyprefs.xls" ts)) :direction :output :if-exists :supersede)
   (if *heuristicated-experiment-label*
       (format o ";;; ~a~%(~%" *heuristicated-experiment-label*)
	   (format o ";;; low=~a, high=~a~%" *low* *high*))
...
(defun summarize-coefs (ts &aux file->coefs)
  ...
  ;; Dump params:
  (with-open-file 
   (o (print (format nil "addstats/~a-mastersummary.xls" ts)) :direction :output :if-exists :supersede)
   ;; Report params
   (if *heuristicated-experiment-label*
       (format o "# ~a~%" *heuristicated-experiment-label*)
     (format o "# low=~a, high=~a~%" *low* *high*))
  ...
  (let ((pns-that-change 
	 (loop for pn being the hash-keys of *params->all-values*
	       ...
	       collect pn)))
    (with-open-file 
     (o (print (format nil "addstats/~a-FinalPivotforR.csv" ts)) :direction :output :if-exists :supersede) 
     (format o "# ~a~%" *heuristicated-experiment-label*) 
     (format o "file")
...
(defun combinalyze (file-numbers cvar mvar)
  (clrhash *c->ms*)
  (loop for (c . m) in 
	(loop for file in (directory "addstats/*-FinalPivotforR.csv")
  ...
  (with-open-file 
   (o (format nil "addstats/~a-combined-analysis.csv" (get-universal-time)) :direction :output)
