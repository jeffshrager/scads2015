...
(defun load-data (low high ts &aux first-fno last-fno label temp constrain-by-label)
  ...
  (with-open-file 
   (logsum (print (format nil "countstats/~a-logsum.lisp" ts))
	   :direction :output :if-exists :supersede) 
   (block load-data-inner ;; Prevent inner return-from from dissappearing the logsum file.
   (loop for file in (downsorted-directory "countruns/*.lisp")
...
(defun summarize (ts)
  ...
  (with-open-file 
   (o (print (format nil "countstats/~a-dcsum.xls" ts))
      :direction :output :if-exists :supersede) 
   (with-open-file 
    (p (print (format nil "countstats/~a-dcnumbers.xls" ts))
       :direction :output :if-exists :supersede) 
  ...
  (with-open-file 
   (o (print (format nil "countstats/~a-npivotsequence.xls" ts))
      :direction :output :if-exists :supersede) 
   (loop for n from 1 to 10
	 do (format o "n~a	" n))
   (format o "~%")
  ...
  (with-open-file 
   (cl-json::*json-output* (print (format nil "countstats/~a-final-encodings.json" ts))
      :direction :output :if-exists :supersede) 
